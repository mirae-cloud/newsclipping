...
- 산업 파급효과 서술에 이어, 경쟁사·동종업계 기업 입장에서의 전략적 시사점(예: 대응 옵션,
  진입장벽·경쟁우위 변화)을 한 문장으로 덧붙일 것 — 분량은 늘리지 말고 기존 한 문단 안에서
  핵심만 압축해 포함할 것.
...
- "M&A / Strategic Investment" vs "Corporate Finance / Turnaround" vs "New Business / Business Building"
  ...
  2. "M&A / Strategic Investment"는 외부의 특정 기업(타사) 지분을 사들이거나 인수·합병하는
     기사만 해당. 자기 자신의 공장·설비 증설 등 내부 역량 강화를 위한 자본적 지출(capex)은
     "투자"라는 단어가 쓰여도 M&A가 아니라 "Strategy"로 분류(예: "Aditya Birla Group, Odisha
     알루미나 정유 공장에 12.6억 달러 투자 확장"은 자사 공장 증설이므로 Strategy).
     신규 시장·사업 진출이 "목적"이어도 지분 인수 형태가 아니면 M&A가 아님
     (예: "한화그룹, 미국 AI 및 방산 시장 본격 진출"처럼 특정 대상 기업의 지분 인수가 아닌 진출
     발표는 M&A가 아니라 New Business).
  3. "Strategy"는 특정 사업부·제품·서비스를 강화(증설·확장 투자)하거나 축소·중단하는 등,
     기존 사업 포트폴리오 구성에 대한 의사결정 기사(자사 설비 투자, 사업부 통폐합, 생산 중단 등).
  4. 기존 사업과 구별되는 신규 사업 영역·아이템·모델 자체를 새로 발굴·착수하는 기사(지역을 막론하고,
     기존에 하지 않던 사업/산업으로 진출하는 경우 포함)는 "New Business / Business Building".
  5. 기존에 하던 동일 사업을 해외/신규 지역·신규 채널·신규 고객층으로 넓히는 기사(예: 기존 제품을
...
- 제외 대상: 실제 발생한 사건·발표·거래가 아니라 개인 의견·논평·전망성 분석이 중심인 기사
  (예: 사설, 칼럼, 주가·시황 전망 분석). 제목에 "오피니언"/"칼럼" 같은 표시가 없어도 내용이
  사실 전달보다 필자의 주관적 견해·전망 위주면 동일하게 제외할 것.
...
- 포함 대상 기사 중에서, 아래 우선순위에 따라 가장 핵심적이고 중요한 기사를 최대 3개까지 선택(selected)할 것.
  화제성·규모가 커 보여도 영향이 오래 지속되지 않는 기사보다, 규모는 작아 보여도 구조적으로 지속되는
  변화를 다루는 기사를 우선할 것:
  1. (최우선) 산업 구조·경쟁 구도의 지속적 변화 — 고객/가입자 구조 변화(예: 가입자 이탈·이동),
     기술 세대 전환(예: 6G 전환 구체화), 신규 인프라·R&D 시설 착공(예: 해저케이블, 데이터센터,
     양자컴퓨팅 캠퍼스), 신제품·신사업 진출처럼 발표 이후에도 영향이 이어지는 변화.
     신제품 자체의 공개·출시(예: 새로운 제품 라인·카테고리 발표, 로봇견 공개 등)는 그 제품이
     계속 판매·영향을 미치는 지속적 변화이므로 이 순위에 해당함.
  2. (차순위) 규모는 크지만 일회성 실적·이벤트 — 신제품 자체가 아니라 그 제품의 판매 실적·반응
     지표(예: 출시 후 매진, 예약 대수)만 다루는 기사, 단발성 실적 발표, 단기 가격 급등락 등.
  3. (최하위, selected에서 우선 배제) 사고·장애·클레임처럼 복구·수습되면 영향이 사라지는 일시적
     이벤트(예: 통신 장애, 일시적 서비스 중단, 해프닝성 사건) — 헤드라인 임팩트가 커 보여도
     1·2번에 해당하는 기사가 있다면 그것을 먼저 선택할 것.
...
def find_cross_category_duplicates(articles: list[dict]) -> list[int]:
    """articles: [{"index": int, "category": str, "title": str}, ...] (한 소스 내 여러 카테고리에 걸쳐
    선택된 기사 전체). 서로 다른 카테고리에 분류됐지만 사실은 같은 사건·발표를 다루는 기사들을 찾아,
    각 그룹에서 가장 적합한 카테고리의 기사 하나만 남기고 나머지의 index를 반환한다."""
    if len(articles) < 2:
        return []

    articles_block = "\n".join(f'{a["index"]}. [{a["category"]}] {a["title"]}' for a in articles)
    prompt = f"""
아래는 여러 카테고리에 걸쳐 오늘 선택된 기사 제목 목록이다. 서로 다른 카테고리에 속해 있지만
사실은 같은 사건·발표·거래를 다루는 기사 그룹이 있는지 찾아라 (예: 같은 M&A 건이 "M&A / Strategic
Investment"와 산업 카테고리 양쪽에 각각 선택된 경우).

같은 사건을 다루는 기사 그룹을 찾으면, 그 사건을 가장 잘 대표하는 카테고리의 기사 하나만
keep_index로 남기고 나머지는 remove_indices에 넣어라. 단순히 같은 산업·회사를 다루는 것만으로는
같은 사건이 아니니, 실제로 동일한 사건·발표를 가리킬 때만 그룹으로 묶을 것. 겹치는 기사가
전혀 없으면 duplicate_groups를 빈 배열로 반환하라.

기사 목록 (번호. [카테고리] 제목):
{articles_block}
"""
    result = _generate_json(prompt, DedupResult)
    return [i for g in result.duplicate_groups for i in g.remove_indices]
